...
const CartItem: React.FC<CartItemProps> = ({ id, title, price, count, imageUrl, type, size }) => {
  ...
  const {sizes, types} = useSelector(appSelector);

  const createParams = (size: number, type: string, pizzaId: string) => {
    const sizeId = sizes.indexOf(size) + 1;
    const typeId = types.indexOf(type) + 1;
    const params = {
  ...
  const onClickPlus = () => {
    ...
    const params = createParams(size, type, id);
    dispatch(addItemToCart(params));
  };

  const onClickMinus = () => {
    /*dispatch(decrementItem(id));*/
    const obj = createParams(size, type, id);
    const params = {
      ...obj,
      deleteAll: false,
    }
