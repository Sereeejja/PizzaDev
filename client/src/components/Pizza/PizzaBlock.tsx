import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import {addItem, addItemToCart, CartItem, cartItemsSelector, fetchCartItemsStatus} from "../../redux/slices/cartSlice";
import {Link} from "react-router-dom";
import {useAppDispatch} from "../../redux/store";

const pizzaTypes = ["Thin", "Traditional"];

type PizzaBlockProps = {
  id: string;
  title: string;
  types: number[];
  imageUrl: string;
  price: number;
  sizes: number[];
}

const PizzaBlock: React.FC<PizzaBlockProps> = ({ id, title, types, imageUrl, price, sizes }) => {
  const [typeIndex, setTypeIndex] = useState<number>(types[0]);
  const [sizeIndex, setSizeIndex] = useState<number>(0);
  const items = useSelector(cartItemsSelector);
  const count = items.filter(item => item.id === id)
      .reduce((acc, item) => acc + item.count, 0);
  const addedCount = count ? count : 0;

  const dispatch = useAppDispatch();
  const onClickAdd = () => {
    /*const item: CartItem = {
      id,
      title,
      price,
      imageUrl,
      type: pizzaTypes[typeIndex],
      size: sizes[sizeIndex],
      count: 0,
    };
    console.log(types)
    dispatch(addItem(item));*/
    const params = {
      pizzaId: Number(id),
      sizeId: sizeIndex + 1,
      typeId: typeIndex + 1,
    }
    dispatch(addItemToCart(params))

  };
  return (
    <div className="pizza-block">
      <Link to={'/pizza/' + id}>
        <img className="pizza-block__image" src={imageUrl} alt="Pizza" />
      </Link>
      <h4 className="pizza-block__title">{title}</h4>
      <div className="pizza-block__selector">
        <ul>
          {types.map((type) => (
            <li
              key={type}
              className={type === typeIndex ? "active" : ""}
              onClick={() => setTypeIndex(type)}
            >
              {pizzaTypes[type]}
            </li>
          ))}
        </ul>
        <ul>
          {sizes.map((size, i) => (
            <li
              key={i}
              onClick={() => setSizeIndex(i)}
              className={i === sizeIndex ? "active" : ""}
            >
              {size} sm.
            </li>
          ))}
        </ul>
      </div>
      <div className="pizza-block__bottom">
        <div className="pizza-block__price">от {price} $</div>
        <div
          onClick={onClickAdd}
          className="button button--outline button--add"
        >
          <svg
            width="12"
            height="12"
            viewBox="0 0 12 12"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
              fill="white"
            />
          </svg>
          <span>Add</span>
          {addedCount > 0 && <i>{addedCount}</i>}
        </div>
      </div>
    </div>
  );
};

export default PizzaBlock;
